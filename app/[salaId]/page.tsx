...
import { ThreatCard } from "../components/ThreatCard"; // Mantido intacto conforme pedido
...
export default function MesaPage() {
  ...
  // -----------------------------------------------------
  // ESTILOS COMUNS (Variáveis para botões premium)
  // -----------------------------------------------------
    ...
    <div className="flex bg-[#0a0a0a] min-h-screen text-gray-100 font-sans overflow-x-hidden bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-gray-900 via-[#0a0a0a] to-black">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-[#050505]/80 backdrop-blur-xl border-b border-white/5 shadow-2xl flex flex-col xl:flex-row justify-between items-center gap-4 py-3 px-6 transition-all duration-300">
          ...
          <div className="flex gap-2 items-center flex-wrap justify-center">
             ...
             <button onClick={() => { if(confirm("Deseja resetar o combate (PV, PM, Condições e Turno)?")) salvarGlobal({ ameacas: ameacas.map(a => ({...a, pvAtual: a.pvMax, pmAtual: a.pmMax, condicoes: []})), turnoIndex: -1, historico: [] }) }} className={`${btnGlass} text-green-100 hover:text-green-300 hover:border-green-500/30`} title="Descanso / Resetar">💤</button>
          </div>
      </header>

      {/* CONTEÚDO PRINCIPAL com PADDING AJUSTADO */}
      <div className={`flex-grow p-4 md:p-8 pt-[200px] md:pt-[100px] w-full transition-all duration-300 ease-in-out ${showLog ? 'xl:mr-80' : ''}`}>
        ...
        {showMap && (
            <div className="mb-8 w-full aspect-video bg-black/80 rounded-2xl overflow-hidden border border-white/10 relative shadow-2xl animate-in fade-in slide-in-from-top-4 ring-1 ring-white/5">
                <BattleMap 
                    ...
                    isGm={true} 
                />
                <div className="absolute bottom-3 right-3 text-[10px] text-gray-400 bg-black/70 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/10 pointer-events-none shadow-lg">
                    Arraste tokens • Ctrl+Scroll p/ Zoom • Scroll no Token p/ Tamanho
                </div>
            </div>
        )}

        {/* TIMELINE (Painel Glassmorphism) */}
        <section className="mb-8 bg-gray-900/40 backdrop-blur-md rounded-2xl border border-white/5 p-5 relative shadow-xl overflow-hidden group/timeline hover:border-white/10 transition-colors">
            ...
            <div className="flex justify-between items-center mb-4 relative z-10">
                <h2 className="text-gray-400 text-xs font-bold uppercase tracking-[0.2em] flex items-center gap-2">
                    <span className="w-2 h-2 bg-blue-500 rounded-full shadow-[0_0_10px_rgba(59,130,246,0.5)]"></span>
                    Ordem de Turno
                </h2>
                <button onClick={proximoTurno} className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-4 py-1.5 rounded-full font-bold transition shadow-lg shadow-blue-900/20 flex items-center gap-1 border border-blue-400/20 active:scale-95">
                    ▶ Próximo Turno
                </button>
            </div>
                ...
                {timeline.map((item, idx) => {
                    ...
                    return (
                        <div key={`${item.tipo}-${item.id}`} className={`flex items-center gap-4 p-3 rounded-xl border transition-all duration-300 relative overflow-hidden group ${isTurno ? 'bg-gradient-to-r from-amber-900/40 to-gray-900/40 border-amber-500/50 shadow-lg shadow-amber-900/10' : 'bg-black/20 border-white/5 hover:bg-white/5 hover:border-white/10'}`} onClick={() => salvarGlobal({ turnoIndex: idx })}>
                            ...
                            <input 
                                type="number" 
                                className={`w-12 bg-black/40 text-center rounded-lg font-bold border text-sm py-1 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all ${isTurno ? 'text-amber-100 border-amber-900/50' : 'text-gray-400 border-white/5'}`}
                            ...
                            <div className="relative">
                                {dadosItem?.imagemUrl ? (
                                    <img src={dadosItem.imagemUrl} className={`w-10 h-10 rounded-full object-cover border-2 shadow-md ${isTurno ? 'border-amber-500 shadow-amber-500/20' : 'border-gray-700'}`} />
                                ) : (
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xl bg-black/40 border-2 ${isTurno ? 'border-amber-500/30 text-amber-500' : 'border-white/5 text-gray-600'}`}>
                                        {item.tipo === 'AMEACA' ? '👾' : '🛡️'}
                                    </div>
                                )}
                            </div>
                            ...
                            {item.tipo === 'JOGADOR' && (
                                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity" onClick={(e) => e.stopPropagation()}>
                                    <UploadButton 
                                        compact
                                        className={`w-7 h-7 flex items-center justify-center rounded-lg border transition text-[10px] ${dadosItem?.imagemUrl ? 'bg-blue-600/20 border-blue-500/50 text-blue-200' : 'bg-gray-800 border-gray-600 text-gray-400 hover:text-white'}`}
                                        label={dadosItem?.imagemUrl ? "Img" : "📷"}
                                    ...
                                    <button 
                                        ...
                                        className="text-gray-500 hover:text-red-400 hover:bg-red-900/20 w-7 h-7 flex items-center justify-center rounded-lg border border-transparent hover:border-red-900/50 transition"
            ...
            <div className="flex gap-3 mt-4 pt-4 border-t border-white/5 items-center relative z-10">
                ...
                <UploadButton 
                    compact
                    className={`p-2 rounded-lg border text-xs font-bold w-12 flex items-center justify-center ${novoAvatarJog ? 'bg-green-900/30 border-green-500/50 text-green-400' : 'bg-gray-800/50 border-white/10 text-gray-500 hover:text-gray-300'}`}
