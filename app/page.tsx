...
import { Ameaca, Jogador, ModeloAmeaca, ResultadoRolagem, ItemTimeline, Atributos } from "./types/game";
...
export default function Page() {
  ...
  useEffect(() => { if(isClient) localStorage.setItem("t20-master-screen-v5", JSON.stringify({ameacas, jogadores})); }, [ameacas, jogadores, isClient]);
  ...
  const addAmeaca = () => {
    ...
    setAmeacas([...ameacas, { 
      ...
      defesa: 10, pvAtual: 10, pvMax: 10, pmAtual: 0, pmMax: 0, acoes: [], pericias: periciasPadrao, atributos: atributosPadrao, condicoes: [],
      iniciativaAtual: combateAtivo ? calcularIniciativa(periciasPadrao) : undefined, imagemUrl: ""
    }]);
  };
  ...
  const processarImportacaoTexto = (dados: Partial<Ameaca>) => {
    ...
    setAmeacas([...ameacas, { 
        ...
        condicoes: [], imagemUrl: "", iniciativaAtual: combateAtivo ? calcularIniciativa(dados.pericias || "") : undefined
    } as Ameaca]);
  ...
  const salvarModelo = (a: Ameaca) => {
    setBestiario(prev => [...prev.filter(b => b.nome !== a.nome), {
      ...
      pvPadrao: a.pvMax, pvMax: a.pvMax, pmPadrao: a.pmMax, pmMax: a.pmMax, acoes: a.acoes, pericias: a.pericias, atributos: a.atributos, imagemUrl: a.imagemUrl
    }]);
  ...
  const importarModelo = (m: ModeloAmeaca) => {
    const combateAtivo = ameacas.some(a => a.iniciativaAtual !== undefined) || jogadores.length > 0;
    setAmeacas([...ameacas, { ...m, id: crypto.randomUUID(), pvAtual: m.pvPadrao, pmAtual: m.pmPadrao, condicoes: [], iniciativaAtual: combateAtivo ? calcularIniciativa(m.pericias) : undefined, imagemUrl: m.imagemUrl || "" }]);
    setMostrarBestiario(false);
  };
  
  const exportar = () => {
    const blob = new Blob([JSON.stringify({ameacas, jogadores, bestiario}, null, 2)], {type: "application/json"});
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "t20-backup-v5.json"; a.click();
  };
